...
enum MissionContentError: LocalizedError {
    case fileNotFound(String), unreadable(String), decodingFailed(String)
    var errorDescription: String? {
        switch self {
        case .fileNotFound(let id): "找不到任务文件：\(id).json"
        case .unreadable(let id): "无法读取任务文件：\(id).json"
        case .decodingFailed(let message): "任务内容格式错误：\(message)"
        }
    }
}
...
final class MissionContentLoader: MissionContentLoading {
    ...
    func loadMission(id: String) throws -> Mission {
        ...
        do { return try JSONDecoder().decode(Mission.self, from: data) }
        catch { throw MissionContentError.decodingFailed(error.localizedDescription) }
    }

    func loadAllMissions() throws -> [Mission] { [try loadMission(id: "apple_decrease_result_001")] }
}

@Model final class PlayerProfileEntity {
    @Attribute(.unique) var id: UUID
    var nickname: String
    init(id: UUID = UUID(), nickname: String = "小调查员") { self.id = id; self.nickname = nickname }
}
...
@Model final class LearningRecordEntity {
    ...
    init(record: LearningRecord) throws {
        id = record.id; playerID = record.playerID; missionID = record.missionID
        payload = try JSONEncoder().encode(record); completedAt = record.completedAt
    }
    func domainValue() throws -> LearningRecord { try JSONDecoder().decode(LearningRecord.self, from: payload) }
}
...
@Model final class SkillMasteryEntity {
    ...
    var skillID: String
    var payload: Data
    init(value: SkillMastery) throws { id = value.id; skillID = value.skillID; payload = try JSONEncoder().encode(value) }
}
...
@MainActor final class SwiftDataLearningRecordRepository: LearningRecordRepository {
    ...
    func save(_ record: LearningRecord) throws { context.insert(try LearningRecordEntity(record: record)); try context.save() }
    func records(for playerID: UUID) throws -> [LearningRecord] {
...
protocol AudioRecordingService { func start() throws; func stop() throws -> URL? }
final class LocalAudioRecordingService: AudioRecordingService { func start() throws {}; func stop() throws -> URL? { nil } }
